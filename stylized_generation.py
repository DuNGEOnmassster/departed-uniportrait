...
def image_to_single_id_generation_process(
        pil_faceid_image_path=None, pil_faceid_supp_images_paths=None,
        pil_faceid_mix_image_1_path=None, mix_scale_1=0.0,
        pil_faceid_mix_image_2_path=None, mix_scale_2=0.0,
        faceid_scale=0.0, face_structure_scale=0.0,
        pil_ip_image_path=None, ip_scale=1.0,
    ...
    single_faceid_cond_kwargs = prepare_single_faceid_cond_kwargs(
        pil_faceid_image=Image.open(pil_faceid_image_path) if pil_faceid_image_path else None,
        pil_faceid_supp_images=pil_faceid_supp_images_paths,
        pil_faceid_mix_images=[pil_faceid_mix_image_1_path, pil_faceid_mix_image_2_path],
        mix_scales=[mix_scale_1, mix_scale_2],
        face_app=face_app
    )
    ...
    if pil_ip_image_path is None:
        raise ValueError("Please provide a path to the portrait reference image.")
    pil_ip_image = Image.open(pil_ip_image_path).convert("RGB")
    h, w = map(int, image_resolution.split("x"))
    pil_ip_image = pil_ip_image.resize((w, h))

    # Inversion
    # Initialize models here to ensure all models are loaded before inversion
    ...
    prompt_list = [""] * (1 + num_samples)
    negative_prompt_list = [""] * (1 + num_samples)
...
def main():
    parser = argparse.ArgumentParser(description="Image-to-Single-ID Stylized Generation")
    parser.add_argument("--pil_ip_image", type=str, required=True, help="Path to portrait reference image")
    parser.add_argument("--pil_faceid_image", type=str, required=True, help="Path to ID image")
    parser.add_argument("--pil_faceid_supp_images", type=str, nargs='*', default=[], help="Paths to additional ID images")
    parser.add_argument("--pil_faceid_mix_image_1", type=str, default=None, help="Path to Mix ID 1 image")
    parser.add_argument("--mix_scale_1", type=float, default=0.0, help="Mix scale 1")
    parser.add_argument("--pil_faceid_mix_image_2", type=str, default=None, help="Path to Mix ID 2 image")
    parser.add_argument("--mix_scale_2", type=float, default=0.0, help="Mix scale 2")
    ...
    parser.add_argument("--ip_scale", type=float, default=0.7, help="Reference Scale")
    parser.add_argument("--num_samples", type=int, default=2, help="Number of images to generate")
    parser.add_argument("--seed", type=int, default=-1, help="Random seed (-1 for random)")
    parser.add_argument("--image_resolution", type=str, default="768x512", help="Image resolution (HxW)")
    ...
    image_to_single_id_generation_process(
        pil_faceid_image_path=args.pil_faceid_image,
        pil_faceid_supp_images_paths=args.pil_faceid_supp_images,
        pil_faceid_mix_image_1_path=args.pil_faceid_mix_image_1,
        mix_scale_1=args.mix_scale_1,
        pil_faceid_mix_image_2_path=args.pil_faceid_mix_image_2,
        mix_scale_2=args.mix_scale_2,
        faceid_scale=args.faceid_scale,
        face_structure_scale=args.face_structure_scale,
        pil_ip_image_path=args.pil_ip_image,
